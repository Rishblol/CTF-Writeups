## Question
Oh Crahp, I forgot my credentials! Can you login nontheless?

## Solution
Here we've been given a site looking like this.
![image](https://github.com/user-attachments/assets/bbc3bc98-d748-4391-b304-8f7762dd6151)

Source code:
```php
<?php
ini_set("error_reporting", 0);
ini_set("display_errors",0);

if(isset($_GET['source'])) {
    highlight_file(__FILE__);
}


// https://www.php.net/manual/en/function.crc32.php#28012
function crc16($string) {
  $crc = 0xFFFF;
  for ($x = 0; $x < strlen ($string); $x++) {
    $crc = $crc ^ ord($string[$x]);
    for ($y = 0; $y < 8; $y++) {
      if (($crc & 0x0001) == 0x0001) {
        $crc = (($crc >> 1) ^ 0xA001);
      } else { $crc = $crc >> 1; }
    }
  }
  return $crc;
}


// https://stackoverflow.com/questions/507041/crc8-check-in-php/73305496#73305496
function crc8($input)
{
$crc8Table = [
    
];

    $byteArray = unpack('C*', $input);
    $len = count($byteArray);
    $crc = 0;
    for ($i = 1; $i <= $len; $i++) {
        $crc = $crc8Table[($crc ^ $byteArray[$i]) & 0xff];
    }
    return $crc & 0xff;
}

$MYPASSWORD = "AdM1nP@assW0rd!";
include "flag.php";

if(isset($_POST['password']) && strlen($MYPASSWORD) == strlen($_POST['password'])) {
    $pwhash1 = crc16($MYPASSWORD);
    $pwhash2 = crc8($MYPASSWORD);

    $password = $_POST['password'];
    $pwhash3 = crc16($password);
    $pwhash4 = crc8($password);

    if($MYPASSWORD == $password) {
        die("oops. Try harder!");
    }
    if($pwhash1 != $pwhash3) {
        die("Oops. Nope. Try harder!");
    }
    if($pwhash2 != $pwhash4) {
        die("OoOps. Not quite. Try harder!");
    }
    $access = true;
 
    if($access) {
        echo "You win a flag: $FLAG";
    } else {
        echo "Denied! :-(";
    }
} else {
    echo "Try harder!";
}
?>
```
```html
<html>
    <head>
        <title>Craphp</title>
    </head>
    <body>
        <h1>Craphp</h1>
        <form action="/" method="post">
            <label for="password">Give me your password!</label><br>
            <input type="text" name="password"><br>
            <button type="submit">Submit</button>
        </form>
        <p>To view the source code, <a href="/?source">click here.</a>
    </body>
</html>
```

Looking at this code, we have to find a word similar in length to `AdM1nP@assW0rd!` having the same crc8 and crc16 values.

Using this code: 
```python
import random
import string

# CRC-16 calculation function
def crc16(data):
    crc = 0xFFFF
    for char in data:
        crc ^= ord(char)
        for _ in range(8):
            if crc & 0x0001:
                crc = (crc >> 1) ^ 0xA001
            else:
                crc >>= 1
    return crc

# CRC-8 calculation function
def crc8(data):
    crc8_table = [
        
    ]

    crc = 0
    for char in data:
        crc = crc8_table[(crc ^ ord(char)) & 0xff]
    return crc & 0xff

# Target string
target_string = "AdM1nP@assW0rd!"
target_length = len(target_string)

# Calculate CRC-16 and CRC-8 of the target string
target_crc16 = crc16(target_string)
target_crc8 = crc8(target_string)

print(f"Target CRC-16: {target_crc16}")
print(f"Target CRC-8: {target_crc8}")

# Function to generate a random string of the same length
def generate_random_string(length):
    characters = string.ascii_letters + string.digits + string.punctuation
    return ''.join(random.choice(characters) for _ in range(length))

# Function to find a matching string
def find_matching_string():
    while True:
        random_string = generate_random_string(target_length)
        random_crc16 = crc16(random_string)
        random_crc8 = crc8(random_string)

        if random_crc16 == target_crc16 and random_crc8 == target_crc8:
            return random_string

# Find and print the matching string
matching_string = find_matching_string()
print(f"Matching String: {matching_string}")
```
we obtain a string `k6bd:95o8Jla6{]` and inputting that we get the flag.

`flag: ENO{Cr4hP_CRC_Collison_1N_P@ssw0rds!}`
